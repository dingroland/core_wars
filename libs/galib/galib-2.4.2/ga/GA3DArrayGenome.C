// $Header: /usr/people/mbwall/src/galib/ga/RCS/GA3DArrayGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  array3.C
  mbwall 25feb95
...
template <class T> void 
GA3DArrayAlleleGenome<T>::copy(const GAGenome& g){
  if(&g == this) return;
  GA3DArrayAlleleGenome<T> & orig = (GA3DArrayAlleleGenome<T>&)g;
  GA3DArrayGenome<T>::copy(g);
  if(naset != orig.naset){
    delete [] aset;
    naset = orig.naset;
...
{
  GA3DArrayGenome<ARRAY_TYPE>& sis=(GA3DArrayGenome<ARRAY_TYPE>&)a;
  GA3DArrayGenome<ARRAY_TYPE>& bro=(GA3DArrayGenome<ARRAY_TYPE>&)b;
