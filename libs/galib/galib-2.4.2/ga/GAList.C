// $Header: /usr/people/mbwall/src/galib/ga/RCS/GAList.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  listtmpl.C
  mbwall 25feb95
...
GAList<T>::~GAList()
{
  while(hd) delete GAListBASE::remove((GANode<T> *)hd);
...
{
  while(hd) delete GAListBASE::remove((GANode<T> *)hd);
  hd = _GAListCopy((GANode<T> *)orig.hd, (GANode<T> *)orig.hd);
  iter.node = hd;
...
GAList<T>::remove()
{
  ...
  node = (GANode<T> *)GAListBASE::remove(node);
...
GAList<T>::clone(unsigned int i) const
{
  ...
  GANode<T> * node;
  unsigned int w=0;
  if(i == 0) node = (GANode<T> *)hd;
  else node = (GANode<T> *)_GAListTraverse(i, w, hd);
  if(!node) return t;

  GANode<T> * newnode = _GAListCopy(node, (GANode<T> *)hd);
