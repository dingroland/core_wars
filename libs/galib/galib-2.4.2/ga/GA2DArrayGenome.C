// $Header: /usr/people/mbwall/src/galib/ga/RCS/GA2DArrayGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  array2.C
  mbwall 25feb95
...
GA2DArrayGenome<T>::copy(const GAGenome & orig){
  ...
  GA2DArrayGenome<T> & c = (GA2DArrayGenome<T> &)orig;
  GAGenome::copy(c);
  GAArray<T>::copy(c);
  nx = c.nx; ny = c.ny; 
...
GA2DArrayGenome<T>::resize(int w, int h){
  if(w == (int)nx && h == (int)ny) return sz;
  ...
  if(w < (int)nx){
    int y=GAMin((int)ny,h);
    for(int j=0; j<y; j++)
      move(j*w,j*nx,w);
  }
  GAArray<T>::size(w*h);
  if(w > (int)nx){		// adjust the existing chunks of bits
    int y=GAMin((int)ny,h);
    for(int j=y-1; j>=0; j--)
...
template <class T> void 
GA2DArrayAlleleGenome<T>::copy(const GAGenome& g){
  if(&g == this) return;
  GA2DArrayAlleleGenome<T> & orig = (GA2DArrayAlleleGenome<T>&)g;
  GA2DArrayGenome<T>::copy(g);
  if(naset != orig.naset){
    delete [] aset;
    naset = orig.naset;
    aset = new GAAlleleSet<T>[orig.naset];
  }
  for(int i=0; i<naset; i++)
...
GA2DArrayAlleleGenome<ARRAY_TYPE>::UniformInitializer(GAGenome & c)
{
  GA2DArrayAlleleGenome<ARRAY_TYPE> &child=
    (GA2DArrayAlleleGenome<ARRAY_TYPE> &)c;
...
{
  GA2DArrayGenome<ARRAY_TYPE> &child=(GA2DArrayGenome<ARRAY_TYPE> &)c;
  register int n, i;
  if(pmut <= 0.0) return(0);
...
{
  GA2DArrayGenome<ARRAY_TYPE>& sis=(GA2DArrayGenome<ARRAY_TYPE>&)a;
  GA2DArrayGenome<ARRAY_TYPE>& bro=(GA2DArrayGenome<ARRAY_TYPE>&)b;
...
UniformCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  GA2DArrayGenome<T> &mom=(GA2DArrayGenome<T> &)p1;
  GA2DArrayGenome<T> &dad=(GA2DArrayGenome<T> &)p2;

  int nc=0;
  int i,j;

  if(c1 && c2){
    GA2DArrayGenome<T> &sis=(GA2DArrayGenome<T> &)*c1;
    GA2DArrayGenome<T> &bro=(GA2DArrayGenome<T> &)*c2;
