// $Header: /usr/people/mbwall/src/galib/ga/RCS/GAListGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  list.C
  mbwall 25feb95
...
GAListGenome<T>::copy(const GAGenome & orig){
  ...
  GAListGenome<T> & c = (GAListGenome<T> &)orig;
  GAGenome::copy(orig);
  GAList<T>::copy(c);
}
...
GAListGenome<T>::write(ostream & os) const 
{
  ...
  os.width(10); os << &(((GANode<T> *)hd)->contents) << "\n";
...
{
  if(this == &c) return 1;
  GAListGenome<T> & b = (GAListGenome<T> &)c;
...
{
  GAListGenome<T> &child=(GAListGenome<T> &)c;
  register int n, i;
...
{
  if(&a == &b) return 0;
  GAListGenome<T>& sis=(GAListGenome<T>&)a;
  GAListGenome<T>& bro=(GAListGenome<T>&)b;

  if(sis.size() > bro.size()) return sis.size() - bro.size();
  if(sis.size() < bro.size()) return bro.size() - sis.size();
