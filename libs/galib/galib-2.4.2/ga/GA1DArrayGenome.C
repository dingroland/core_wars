// $Header: /usr/people/mbwall/src/galib/ga/RCS/GA1DArrayGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  array1.C
  mbwall 25feb95
...
GA1DArrayGenome<T>::copy(const GAGenome & orig){
  ...
  GA1DArrayGenome<T> & c = (GA1DArrayGenome<T> &)orig;
  GAGenome::copy(c);
  GAArray<T>::copy(c);
  nx = c.nx; 
...
GA1DArrayGenome<T>::resize(int len)
{
  ...
  if(len == GAGenome::ANY_SIZE)
    ...
    if(len < (int)minX) len=minX;
    if(len > (int)maxX) len=maxX;
  }
...
GA1DArrayGenome<T>::equal(const GAGenome & c) const {
  GA1DArrayGenome<T> & b = (GA1DArrayGenome<T> &)c;
...
template <class T> void 
GA1DArrayAlleleGenome<T>::copy(const GAGenome& g){
  if(&g == this) return;
  GA1DArrayAlleleGenome<T> & orig = (GA1DArrayAlleleGenome<T>&)g;
  GA1DArrayGenome<T>::copy(orig);
  if(naset != orig.naset){
    delete [] aset;
    naset = orig.naset;
...
GA1DArrayAlleleGenome<ARRAY_TYPE>::UniformInitializer(GAGenome & c)
{
  GA1DArrayAlleleGenome<ARRAY_TYPE> &child=
    (GA1DArrayAlleleGenome<ARRAY_TYPE> &)c;
...
{
  GA1DArrayGenome<ARRAY_TYPE>& sis=(GA1DArrayGenome<ARRAY_TYPE>&)a;
  GA1DArrayGenome<ARRAY_TYPE>& bro=(GA1DArrayGenome<ARRAY_TYPE>&)b;
...
UniformCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  GA1DArrayGenome<T> &mom=(GA1DArrayGenome<T> &)p1;
  GA1DArrayGenome<T> &dad=(GA1DArrayGenome<T> &)p2;

  int n=0;
  int i;

  if(c1 && c2){
    GA1DArrayGenome<T> &sis=(GA1DArrayGenome<T> &)*c1;
    GA1DArrayGenome<T> &bro=(GA1DArrayGenome<T> &)*c2;
    ...
    GA1DArrayGenome<T> &sis = 
      (c1 ? (GA1DArrayGenome<T> &)*c1 : (GA1DArrayGenome<T> &)*c2);
...
PartialMatchCrossover(const GAGenome& p1, const GAGenome& p2, 
  ...
  if(c1 && c2){
    ...
    for(i=a, index=a; i<b; i++, index++){
      for(j=0; j<sis.length()-1 && sis[j] != dad.gene(index); j++);
      sis.swap(i,j);
    }
    bro.GAArray<T>::copy(dad);
    for(i=a, index=a; i<b; i++, index++){
      for(j=0; j<bro.length()-1 && bro[j] != mom.gene(index); j++);
      bro.swap(i,j);
    }
...
GA1DArrayIsHole(const GA1DArrayGenome<T> &c, const GA1DArrayGenome<T> &dad,
  ...
  for(int i=a; i<b; i++)
    if(c[index] == dad.gene(i)) return 1;
  return 0;
}
...
// Now put the 'holes' in the proper order within the crossover section.
    for(i=a; i<b; i++){
      if(sis[i] != dad.gene(i)){
	for(j=i+1; j<b; j++)
	  if(sis[j] == dad.gene(i)) sis.swap(i,j);
      }
    }
...
// Now put the 'holes' in the proper order within the crossover section.
	...
	for(j=i+1; j<b; j++)
	  if(bro[j] == mom.gene(i)) bro.swap(i,j);
      }
    }
    ...
    for(i=a; i<b; i++){
      if(sis[i] != parent2->gene(i)){
	for(j=i+1; j<b; j++)
	  if(sis[j] == parent2->gene(i)) sis.swap(i,j);
      }
    }
...
CycleCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  if(c1 && c2){
    ...
    for(i=0; i<sis.size(); i++)
      if(mask[i] == 0) sis[i] = parent2->gene(i);
