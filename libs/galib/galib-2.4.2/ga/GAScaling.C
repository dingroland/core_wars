// $Header: /usr/people/mbwall/src/galib/ga/RCS/GAScaling.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  scaling.C
  mbwall 10aug94
...
GASharing::copy(const GAScalingScheme & arg){
  if(&arg != this && sameClass(arg)){
    ...
    _minmax = ((GASharing&)arg)._minmax;
    _sigma = ((GASharing&)arg)._sigma;
    _alpha = ((GASharing&)arg)._alpha;
    df = ((GASharing&)arg).df;
    N = ((GASharing&)arg).N;
    d = new float[N*N];
    memcpy(d, ((GASharing&)arg).d, N*N*sizeof(float));
  }
}
