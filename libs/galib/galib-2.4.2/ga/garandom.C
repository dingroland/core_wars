// $Header: /usr/people/mbwall/src/galib/ga/RCS/garandom.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  random.C
  mbwall 5sep95
...
//   Loop through the bits to make sure that we don't lose information if the
// system happens to return longs from time (we use unsigned ints for the seed)
// because we don't want to look at only the most significant bits (or always
// least significant bits).  If long int is same number of bits as unsigned
// int then all we've done is scramble the bits, so no harm done.
//   There is an outside chance that we could end up with an infinite loop here
// but the likeliehood of that happening is pretty small.
//   It turns out that different seeds can result in the same random number
// sequence.  I'm not sure exactly why, but the result is that you can get
// identical runs from different seeds (probably a casting problem in the RNG).
void 
GARandomSeed(unsigned s){
  ...
  if(s == 0 && seed == 0) {
	...
	if(i & 1)			// alternately get MSB or LSB
	  seed += (tmp & (1 << i));
	else
	  seed += (tmp & (1 >> i));
      }
