// $Header: /usr/people/mbwall/src/galib/ga/RCS/GAStatistics.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  statistics.C
  mbwall 28jul94
...
GAStatistics::update(const GAPopulation & pop){
  ...
  double tmpval;
  tmpval = ((double)on*(curgen-1)+(double)(pop.ave()))/curgen;
  on = tmpval;
  tmpval = ((double)offmax*(curgen-1)+(double)(pop.max()))/curgen;
  offmax = tmpval;
  tmpval = ((double)offmin*(curgen-1)+(double)(pop.min()))/curgen;
  offmin = tmpval;
...
//  setConvergence(maxScore[0]);
  ...
  divCur = divInit = ((dodiv == gaTrue) ? pop.div() : -1.0);
...
GAStatistics::setScore(const GAPopulation& pop){ 
  ...
  minCur = pop.min();
  devCur = pop.dev();
  divCur = ((dodiv == gaTrue) ? pop.div() : -1.0);
