// $Header: /usr/people/mbwall/src/galib/ga/RCS/GA1DBinStrGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  binstr1.C
  mbwall 19apr95
...
GA1DBinaryStringGenome::copy(const GAGenome & orig)
{
  ...
  GA1DBinaryStringGenome & c = (GA1DBinaryStringGenome &)orig;
  GAGenome::copy(orig);
...
GA1DBinaryStringGenome::resize(int l)
{
  ...
  if(l == GAGenome::ANY_SIZE)
    ...
    if(l < (int)minX) l=minX;
    if(l > (int)maxX) l=maxX;
  }
...
GA1DBinaryStringGenome::equal(const GAGenome & c) const {
  GA1DBinaryStringGenome & b = (GA1DBinaryStringGenome &)c;
  return((this == &c) ? 1 : ((nx != b.nx) ? 0 :
			     GABinaryString::equal(b,0,0,nx)));
}
...
UniformCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  if(c1 && c2){
    ...
    GA1DBinaryStringGenome &sis =
      (c1 ? (GA1DBinaryStringGenome &)*c1 : (GA1DBinaryStringGenome&)*c2);
