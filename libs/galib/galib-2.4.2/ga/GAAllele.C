// $Header: /usr/people/mbwall/src/galib/ga/RCS/GAAllele.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  allele.C
  mbwall 21mar95
...
//   If you use this on a complex object you probably won't have the arithmetic
// operators defined for your class, so this will generate compiler errors
// when you try to compile.  In those cases you must specialize this function
// so that it makes sense for the object you are using.
template <class T> T
GAAlleleSet<T>::allele() const {
  if(core->type == GAAllele::ENUMERATED)
    ...
    return core->a[0] + (T)GARandomFloat(0,n)*core->a[2];
  }
  else{
    return (T)GARandomFloat(core->a[0], core->a[1]);
  }
}
