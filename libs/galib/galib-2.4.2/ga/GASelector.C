// $Header: /usr/people/mbwall/src/galib/ga/RCS/GASelector.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  selector.C
  mbwall 10aug94
...
GARouletteWheelSelector::update() {
  ...
  if(which == GASelectionScheme::RAW){
	...
	psum[0] = pop->individual(0).score();
	for(i=1; i<n; i++)
	  psum[i] = pop->individual(i).score() + psum[i-1];
	for(i=0; i<n; i++)
	  psum[i] /= psum[n-1];
      }
      else{
	psum[0] = -pop->individual(0).score() + pop->max() + pop->min();
	for(i=1; i<n; i++)
	  psum[i] = -pop->individual(i).score() + pop->max() + pop->min()
	    + psum[i-1];
	for(i=0; i<n; i++)
	  psum[i] /= psum[n-1];
      }
	...
	psum[i] = (float)(i+1)/(float)n;	// equal likelihoods
      ...
      if(pop->order() == GAPopulation::HIGH_IS_BEST){
	psum[0] = pop->individual(0).fitness();
	for(i=1; i<n; i++)
	  psum[i] = pop->individual(i).fitness() + psum[i-1];
	for(i=0; i<n; i++)
	  psum[i] /= psum[n-1];
      }
      else{
	psum[0]= -pop->individual(0).fitness() + pop->fitmax() + pop->fitmin();
	for(i=1; i<n; i++)
	  psum[i] = -pop->individual(i).fitness() + 
...
GASRSSelector::update() {
  ...
  if(which == GASelectionScheme::RAW){
	...
	if(pop->order() == GAPopulation::HIGH_IS_BEST)
	  expected = pop->individual(i).score()/pop->ave();
	else
	  expected = 
	    (-pop->individual(i).score() + pop->max() + pop->min())/pop->ave();
	ne = (int)expected;
	...
	choices[i] = GARandomInt(0, n-1);
      ...
      for(i=0; i<pop->size(); i++){
	if(pop->order() == GAPopulation::HIGH_IS_BEST)
	  expected = pop->individual(i).fitness()/pop->fitave();
	else
	  expected = 
	    (-pop->individual(i).fitness() + pop->fitmax() + pop->fitmin()) /
	      pop->fitave();
	ne = (int)expected;
	fraction[i] = expected - ne;
	while(ne > 0){
