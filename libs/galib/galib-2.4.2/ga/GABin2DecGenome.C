// $Header: /usr/people/mbwall/src/galib/ga/RCS/GABin2DecGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  bin2dec.C
  mbwall 19apr95
...
GABin2DecGenome::copy(const GAGenome & orig)
{
  ...
  GABin2DecGenome & c = (GABin2DecGenome &)orig;
...
{
  if(n >= (unsigned int)ptype->nPhenotypes()){
...
{
  float value;
  for(int i=0; i<phenotypes().nPhenotypes(); i++){
    is >> value;
