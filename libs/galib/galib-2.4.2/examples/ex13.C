...
// Now create the GA and run it.
  ...
  GA2DBinaryStringGenome
    picture_genome(data.width, data.height, PictureObjective, (void *)&data);
  GABin2DecGenome numbers_genome(map);
...
// then print them out.
  ...
  numbers_genome.copy(*data.numbers_genome);
...
PictureObjective(GAGenome & c)
{
  ...
  if(value/(genome.width()*genome.height()) < 0.95) return(value);

  data->picture_genome->copy(c);
//  *data->picture_genome = genome;
  data->picture_genome->evaluator(NumbersObjective);
  GAIncrementalGA ga(*(data->numbers_genome));
  ...
  *data->numbers_genome = ga.statistics().bestIndividual();
  value += 10 * data->numbers_genome->score();
